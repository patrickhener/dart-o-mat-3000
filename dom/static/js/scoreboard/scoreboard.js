//Imports
import playSound from '../helper/helper.js';
...
socket.on('highlightActiveCricket', function(
  player,
  playerID,
  round,
  message,
...
socket.on('highlightActive', function(
  player,
  playerID,
  round,
  message,
...
function drawScoreboardCricket(list, lastthrows, closed) {
  ...
  for (var item in list) {
    ...
    playerNameColumn.innerHTML =
      "<h2 id='playerName'>" + list[item].Player + '</h2>';
    ...
    scoreColumn.innerHTML =
      "<h2 id='playerScore'>" + list[item].Score + '</h2>';
...
function highlightActiveCricket(
  activePlayer,
  playerID,
  playerRound,
  message,
...
function drawScoreboardATC(list) {
  ...
  for (var item in list) {
    ...
    numberDiv.innerHTML =
      "<h1 id='playerNumber'>" + list[item].Number + '</h1>';
